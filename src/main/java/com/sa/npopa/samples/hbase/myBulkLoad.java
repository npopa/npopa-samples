...
export HBASE_CLASSPATH=SF-HBbulkload-samples-0.0.1-SNAPSHOT.jar 


#0) generate input files (locally) - you need to trensfer them to HDFS for next step.

hbase com.sa.npopa.samples.util.GenerateSampleFiles \
   --outputPath /tmp \
   --numRecords 100000

hdfs dfs -put /tmp/col1 /tmp/mybulkload/col1
hdfs dfs -put /tmp/col2 /tmp/mybulkload/col2

#1) generate hfiles for the input files
export INPUT=/tmp/mybulkload/col1
export OUTPUT=/tmp/myhfiles
hdfs dfs -rm -r -skipTrash  $OUTPUT 
hbase com.sa.npopa.samples.myBulkLoad -libjars /opt/cloudera/parcels/CDH/jars/json-simple-1.1.1.jar \
   --input $INPUT \
   --output $OUTPUT \
   --tableName t1 \
   --columnFamily f1 \
...
#check that the hfiles were generated 
hadoop fs -ls $OUTPUT 

#2) bulkload the hfiles to hbase
hbase org.apache.hadoop.hbase.mapreduce.LoadIncrementalHFiles $OUTPUT  t1

#check the number of rows in hbase
...
public class myBulkLoad extends Configured implements Tool {
  ...
  private static final Log LOG = LogFactory.getLog(myBulkLoad.class);
	private static String table_name;
	private static String family;
	private static String column;
	private static String rowkey_attribute;
	...
	public myBulkLoad(Configuration conf) {
		super(conf);
		init();
	}
  ...
  public static void main(String[] args) throws Exception {
    int exitCode = ToolRunner.run(new myBulkLoad(), args);
